...
本项目旨在一定程度上解决这一问题。本项目的目标是开发一个自动化、高效、可定制的测试用例生成器。该生成器能够解析以C11语法子集定义的变量及其约束关系，并自动生成大量满足（或不满足）这些约束的测试数据。
...
1. 类C11语法文本输入：一个自定义的、以C语言语法为基础的约束描述文件。这个文件包含：
  ...
  - 特殊函数声明: \_LENGTH, GAUSSIAN。这些不是标准的C函数，而是对约束系统的“约束原语”，需要特殊处理。
  - 约束函数: \_CONSTRAINT() 函数体内的表达式集合，它们是核心的约束规则。
2. 约束条件：
  - 布尔与算术约束: a > 5 && a < 10, a + b[0] != s[0].a。这些约束可以使用SMT（Satisfiability Modulo Theories）求解器处理，也可以使用模糊测试方法并基于某些启发式算法求解@borzacchiello_fuzzing_2021。
  - 数组/结构体约束: 约束涉及数组元素 (b[1]) 和结构体成员 (s[0].a)，且两者可以嵌套（s[0].arr[1]）并支持多维数组(s[0][1][2])。
  - 约束原语:
    - \_LENGTH(s[0].d) > 6: 约束对s[0].d指向的内存区域的“长度”进行约束。这实际上是在定义一个动态分配的数组的长度。
    - GAUSSIAN(s[0].c, 1.0, 1.0): 是一个生成性约束或概率分布约束。它指示变量s[0].c的值应该从一个高斯分布$G(mu=1.0,sigma=1.0)$中采样。
3. 输出：
  - 正例：一组满足所有约束的变量赋值。
...
#h(2em)本项目的难点在于首先，如何将这个C11-like的文本文件，准确地转换成机器可处理的结构？首先，本项目需要解析变量声明、类型定义和约束表达式。其次， 如何将不同类型的约束（算术、逻辑、元约束）统一到一个模型中，特别是 \_LENGTH 和 GAUSSIAN 这种约束原语，Z3@de_moura_z3_2008\等标准求解器无法直接理解。以及如何系统性地、有针对性地生成违反特定约束的用例，而不是随机地产生一个无效数据。
...
系统采用流水线式的功能架构，将复杂的生成任务分解为一系列独立的、顺序执行的模块。多线程机制则基于同时启动多个样例生成流水线然后并行运行各流水线。

1.  命令行解析模块：负责解析用户输入的参数（如 `-n`, `-p`, `-c`），为后续模块提供配置。命令行参数帮助信息如下所示,其意义如@tlb:cmd\所示。
```
usage: D:\college2025spr\SoftwareAnalysTest\saturday-testgen\.cmake_build\main.exe --num_cases=int --pos_ratio=double --cons=string [options] ...
options:
...
3.  约束解析模块 (ANTLR)：
    -   利用ANTLRv4生成的C11语法分析器，将输入的约束文件（`.c`）解析为一棵抽象语法树（AST）。这是对C代码进行结构化理解的第一步。ANTLRv4是Java实现的解析器生成器，除了原生支持的Java之外，它也提供了C++、Python等语言的运行时。在提供了EBNF文法(\*.g4文件,ANTLR仓库提供了常见语言，包括C11的文法定义)后，它可以生成对应语言实现的parser。
    - 获得ANTLR解析生成的语法树后，使用ANTLR Visitor API可以访问语法树中的各节点。即访问器模式继承Visitor接口实现对应各语法规则的虚函数以实现实际处理逻辑。
4.  符号化转换模块 (AST Visitor)：
    -   核心模块，通过遍历AST，将C代码中的实体和逻辑转换为Z3 SMT Solver@de_moura_z3_2008\维护的约束表达式。
    -   变量与符号表：识别C代码中的变量声明（如 `int a`, `S1 s[2]`），在内部维护一个符号表（`SymbolTable`），记录每个变量的名称、类型、维度等信息，并为其创建对应的Z3符号变量。
    -   约束与Z3表达式：遍历 `_CONSTRAINT()` 函数体内的表达式，将C的算术、关系和逻辑运算逐一翻译成等价的Z3约束表达式（`z3::expr`）。例如，C代码 `a > 5 && b[0] < 10` 会被转换为Z3中的逻辑与表达式 `And(a > 5, b_0 < 10)`，这个转换的过程是通过在语法树上递归自底向上依次构建子表达式实现的。
...
本项目使用C++17实现，使用CMake#footnote[https://cmake.org/]作为构建工具，vcpkg#footnote[https://vcpkg.io/]用于包管理，C++17可以满足本项目对于内存访问和线程细粒度控制的需求，这允许本项目达到更好的性能。本项目的实现依赖于一系列业界成熟的第三方库：

1.   ANTLRv4#footnote[https://www.antlr.org/]：一个强大的语法分析器生成器，用于解析C11语法，构建AST。ANTLRv4解析EBNF文法生成parser源代码而不是解释执行EBNF文法定义，源代码最终会被编译为原生二进制程序，这为其提供了较好的性能。
2.   Z3 SMT Solver #footnote[https://github.com/Z3Prover/]：世界领先的约束满足理论（SMT）求解器，负责处理所有符号计算和约束求解任务。本项目直接调用Z3++.h API与Z3求解器交互，最大程度地节省了约束求解的开销。
3.   QuickJS#footnote[https://bellard.org/quickjs/]：一个轻量、快速的嵌入式JavaScript引擎，用于在C++环境中执行独立的用例验证逻辑,实际使用了QuickJS-NG分支以支持在Windows下运行。
4.   nlohmann/json#footnote[https://github.com/nlohmann/json]：一个现代C++的JSON库，用于方便地生成和处理JSON数据。本项目最终输出的样例使用JSON文档表示。
5.   cmdline.h#footnote[https://github.com/tanakh/cmdline]：一个头文件式的C++命令行参数解析库。
...
1.  动态代码生成：在 `writeCases` 函数中，系统将原始C约束文件中的约束表达式（`m_cons_expressions`）转换为JavaScript布尔表达式字符串。
...
var f = () => {
    ...
    Object.assign(this, {"a":6,"b":[0,0,-1],"s":[{"a":7,"b":0,"c":2.0,"d":[14,15,16,17,18,19,0]},{"a":11,"b":0,"c":3.0,"d":[14,15,16,17,18,19,0]}]});
    return (a>5&&a<10) && (b[1]>b[2]||b[0]<b[1]) && (a+b[0]!=s[0].a) && (_LENGTH(s[0].d)>6&&_LENGTH(s[0].d)<10) && (_LENGTH(s[1].d)>6&&_LENGTH(s[1].d)<10) && (s[0].d[6]+s[1].d[6]==s[0].b*s[1].b);
};
f();
...
功能测试旨在验证工具是否能正确解析和处理C11语法子集中的各种变量声明和约束逻辑。我们编写了一系列具有针对性的约束文件，每个文件侧重于测试系统的某个特定方面，如@tlb:func-test\所示。
